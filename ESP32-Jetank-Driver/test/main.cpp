...
void setup()
{
    ...
    slam_data.ticks_per_rev = 250;
    slam_data.wheel_diameter = 6.5f;
    slam_data.wheel_base = 12.5f;
...
void connectToWiFi()
{
    // WiFi.config(IPAddress(192,168,1,25), IPAddress(255, 255, 255, 0), IPAddress(192, 168, 1, 1), IPAddress(8, 8, 8, 8));
...
void TaskIMU(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        {
            IMU.readSensor();
            slam_data.accel_x = IMU.getAccelX_mss();
            slam_data.accel_y = IMU.getAccelY_mss();
            slam_data.accel_z = IMU.getAccelZ_mss();
            slam_data.gyro_x = IMU.getGyroX_rads();
            slam_data.gyro_y = IMU.getGyroY_rads();
            slam_data.gyro_z = IMU.getGyroZ_rads();
            slam_data.mag_x = IMU.getMagX_uT();
            slam_data.mag_y = IMU.getMagY_uT();
            slam_data.mag_z = IMU.getMagZ_uT();
            slam_data.temp = IMU.getTemperature_C();
            slam_data.timestamp = millis() / 1000.0;

            if (IS_OK(lidar.waitPoint()))
            {
                slam_data.distance = lidar.getCurrentPoint().distance;
                slam_data.angle = lidar.getCurrentPoint().angle;
                slam_data.quality = (uint8_t)lidar.getCurrentPoint().quality;
            }
            else
            {
            ...
            tcpClient.sendPacket(0x01, (uint8_t *)&slam_data, sizeof(slam_data));
...
{
    slam_data.left += leftEnc;
    slam_data.right += rightEnc;
}
